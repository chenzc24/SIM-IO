...
**Internal flow:**
1. Export a fresh Spectre netlist from the testbench schematic.
2. Build `deck.scs` from `sim_config.json` plus site model includes.
3. Run Spectre directly through `SpectreSimulator`.
4. Parse PSF results locally into `measurements.json` and SVG plots.
5. Sync the same resolved settings into Maestro for GUI inspection.
...
- [ ] symbol_export exit 0: `pin_info.json` written, `.latest_run` updated
...
- [ ] LLM: `tstop` computed from max vpulse `per` x 10
- [ ] LLM: `sim_config.json` written to `<run_dir>/` with `pin_measurements` for every DUT pin
- [ ] testbench_build exit 0: `{cell}_tb/schematic` created in Virtuoso
- [ ] testbench_build: `result.json` written to run directory
- [ ] Sim (if requested): `sim_run_result.json`, `measurements.json`, and SVG plots present in run directory
- [ ] Maestro sync: `sim_run_result.json` has `maestro_sync.maestro_simulation_run = false`
